...
import {
  EFOOPRESALE_CONTRACT_ADDRESS,
  EFOOPRESALE_CONTRACT_ABI_URL,
  EFOOPRESALE_CONTRACT_NAME,
  EFOO_TOKEN_DECIMALS,
  EFOO_TOKEN_TICKER,
} from 'config';
import {
...
const Presale = () => {
  ...
  React.useEffect(() => {
    (async () => {
      const registry = await AbiRegistry.load({ urls: [EFOOPRESALE_CONTRACT_ABI_URL] });
      const abi = new SmartContractAbi(registry, [EFOOPRESALE_CONTRACT_NAME]);
      const contract = new SmartContract({ address: new Address(EFOOPRESALE_CONTRACT_ADDRESS), abi: abi });
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const exchange_rate = convertWeiToEsdt(value.token_price_rate, EFOO_TOKEN_DECIMALS) / convertWeiToEsdt(value.egld_price_rate);
      ...
      const token_sale_amount = convertWeiToEsdt(value.token_sale_amount, EFOO_TOKEN_DECIMALS);
      const total_bought_amount_in_egld = convertWeiToEsdt(value.total_bought_amount_in_egld);
      const total_bought_amount_in_esdt = convertWeiToEsdt(value.total_bought_amount_in_esdt, EFOO_TOKEN_DECIMALS);
  ...
  async function buyToken() {
    const tx = {
      ...
      data: 'buy',
      receiver: EFOOPRESALE_CONTRACT_ADDRESS,
      gasLimit: 10000000,
    };
            ...
            <div className='custom-presale-left'>
              ...
              <div className='custom-presale-body'>
                ...
                <div className='custom-progress-container'>
                  ...
                  <div className='custome-progress-number color-white'>{presale?.total_bought_amount_in_esdt} / {presale?.token_sale_amount} {EFOO_TOKEN_TICKER}</div>
                </div>

                <div className='custom-presale-price'>1 EGLD = {presale?.exchange_rate} {EFOO_TOKEN_TICKER}</div>
              ...
              <div className='custom-buy-card-body'>
                ...
                <div className='custom-buy-card-amount'>
                  ...
                  <div className='custom-buy-card-amount-container'>
                    ...
                    <span className='custom-buy-card-amount-unit color-white'>{EFOO_TOKEN_TICKER}</span>
                  </div>
                </div>
                ...
                <div style={{ paddingTop: '32px' }}>
                  <button className="custom-buy-card-buy-button" onClick={buyToken} disabled={buyButtonDisabled}>Buy {EFOO_TOKEN_TICKER}</button>
