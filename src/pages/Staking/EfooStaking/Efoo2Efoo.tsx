...
import EFOOLogo from 'assets/img/token logos/EFOO.svg';
import AlertModal from '../../../components/AlertModal';

import {
  EFOO2EFOO_CONTRACT_ADDRESS,
  EFOO2EFOO_CONTRACT_ABI,
  EFOO2EFOO_CONTRACT_NAME,
  EFOO_TOKEN_TICKER,
  EFOO_TOKEN_ID,
  EFOO_TOKEN_DECIMALS,
  EFOO_CLAIM_PERIOD_IN_DAYS,
} from 'config';
...
const EFOO2EFOO = () => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const registry = await AbiRegistry.load({ urls: [EFOO2EFOO_CONTRACT_ABI] });
      const abi = new SmartContractAbi(registry, [EFOO2EFOO_CONTRACT_NAME]);
      const contract = new SmartContract({ address: new Address(EFOO2EFOO_CONTRACT_ADDRESS), abi: abi });
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const min_stake_limit = convertWeiToEsdt(value.min_stake_limit, EFOO_TOKEN_DECIMALS);
      ...
      const total_staked_amount = convertWeiToEsdt(value.total_staked_amount, EFOO_TOKEN_DECIMALS);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const staked_amount = convertWeiToEsdt(value.staked_amount, EFOO_TOKEN_DECIMALS);
      const lock_end_timestamp = value.lock_end_timestamp.toNumber();
      const unstaked_amount = convertWeiToEsdt(value.unstaked_amount, EFOO_TOKEN_DECIMALS);
      const undelegation_end_timestamp = value.undelegation_end_timestamp.toNumber();
      const collectable_amount = convertWeiToEsdt(value.collectable_amount, EFOO_TOKEN_DECIMALS);
      const reward_amount = convertWeiToEsdt(value.reward_amount, EFOO_TOKEN_DECIMALS);
  ...
  React.useEffect(() => {
    if (account.address) {
      axios.get(`${network.apiAddress}/accounts/${account.address}/tokens?search=${EFOO_TOKEN_TICKER}`).then((res: any) => {
        let _balance = 0;
        if (res.data?.length > 0) {
          const tokens = res.data.filter(
            (a: any) => a?.identifier === EFOO_TOKEN_ID
          );

          if (tokens.length > 0) {
            // console.log('tokens[0]', tokens[0]);
            _balance = convertWeiToEsdt(tokens[0].balance, EFOO_TOKEN_DECIMALS);
          }
        }
  ...
  function onModalInputAmountChange(value: any) {
    ...
    if (isStakeModal) { // stake
      if (value > balance) {
        ...
        _modalInfoMesssage = `Cannot stake less than ${stakeSetting.min_stake_limit} ${EFOO_TOKEN_TICKER}.`;
      } else {
  ...
  async function stake(e: any) {
    ...
    if (balance == 0) {
      onShowAlertModal(`You don\'t have ${EFOO_TOKEN_TICKER} in your wallet.`);
      return;
    }

    const args: TypedValue[] = [
      BytesValue.fromUTF8(EFOO_TOKEN_ID),
      new BigUIntValue(convertEsdtToWei(modalInputAmount, EFOO_TOKEN_DECIMALS)),
    ...
    const tx = {
      receiver: EFOO2EFOO_CONTRACT_ADDRESS,
  ...
  return (
    <div className='card'>
      <div className='stake_earn'>
        <div>
          <img src={EFOOLogo} />
          <p className='heading'>Stake $EFOO</p>
        </div>
        <img src={arrow} />
        <div>
          <img src={EFOOLogo} />
          <p className='heading'>Earn $EFOO</p>
        </div>
      </div>
      ...
      <div className='info' style={{ marginTop: "8px" }}>
        ...
        <div>
          ...
          <p className='data'>{stakeSetting ? convertAPR2APY(stakeSetting.apr, EFOO_CLAIM_PERIOD_IN_DAYS) : '-'} %</p>
        </div>
        <div>
          <p className='heading'>Total Staked</p>
          <p className='data'>{stakeSetting ? stakeSetting.total_staked_amount : '-'} {EFOO_TOKEN_TICKER}</p>
        </div>
        <div>
      ...
      <div className='info'>
        <div>
          ...
          <p className='data'>{stakeAccount ? stakeAccount.staked_amount : '-'} EFOO</p>
        </div>
        <div>
          <p className='heading'>My Unstaked</p>
          <p className='data'>{stakeAccount ? stakeAccount.unstaked_amount : '-'} EFOO</p>
        </div>
        <div>
          <p className='heading'>My Reward</p>
          <p className='data'>{stakeAccount ? stakeAccount.reward_amount : '-'} EFOO</p>
        </div>
        <div>
          <p className='heading'>My Collectable</p>
          <p className='data'>{stakeAccount ? stakeAccount.collectable_amount : '-'} EFOO</p>
        </div>
      </div>
