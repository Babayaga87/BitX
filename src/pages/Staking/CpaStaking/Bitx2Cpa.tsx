...
import CpaLogo from 'assets/img/token_logos/CPA.png';
...
const Btx2Cpa = () => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const max_stake_limit = convertWeiToEsdt(value.max_stake_limit, HETO_TOKEN_DECIMALS);
  ...
  React.useEffect(() => {
    if (!address) return;
    getBalanceOfToken(network.apiAddress, account, HETO_TOKEN_ID).then((v) => setBalance(v));
  ...
  function onModalInputAmountChange(v: any) {
    ...
    if (isStakeModal) { // stake
      if (value <= 0) {
        ...
        _modalInfoMesssage = `Cannot stake more than ${Math.max(stakeSetting.max_stake_limit - stakeAccount.staked_amount, 0)} ${HETO_TOKEN_TICKER}.`;
      } else {
  ...
  return (
    <div className='card'>
THIS POOL WILL BE CLOSED STARTING 15.01.2023 !
